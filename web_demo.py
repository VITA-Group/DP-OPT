...
def train_interface(dataset, model, algorithm, eps, device):
    from train_opt import main
    
    if algorithm.lower() == "opt":
        args = f"--ape_mode=iid_ibwd --ensemble_gen=True --gen_temp=1.1 --num_prompt=40 --max_new_tokens=50 --data={dataset} --holdout_ratio=0.01 --target_eps=8. --dp_eps=1.8 --dp_delta=5e-7 --tokenwise_gen=True --model={model} --device={device} --no_wandb"
    elif algorithm.lower() == "dp-opt":
        args = f"--ape_mode=iid_ibwd --ensemble_gen=True --ensemble_num=205 --gen_temp=1.1 --num_prompt=40 --max_new_tokens=50 --data={dataset} --holdout_ratio=0.01 --batch_size=12 --target_eps=8. --dp_eps={eps} --dp_delta=5e-7 --tokenwise_gen=True --device={device}  --no_wandb"
    elif algorithm.lower() == "dln-1":
        args = f"--ape_mode=bwd --gen_temp=0.7 --num_demos=20 --num_prompt=20 --rm_eval_item_name=True --steps=20 --max_new_tokens=100 --data={dataset} --holdout_ratio=0.01 --batch_size=32 --device={device} --no_wandb"
    else:
