"""Codes from DLN to load datasets."""
import json
import os
...
class Dataset:
    def __init__(
        ...
        self.dataset_info = self._load_config(
            ...
            pjoin(dataset_path, '..', "dataset_info.yaml")
        )
    ...
    def _load_config(config_file):
        assert os.path.exists(config_file), f"Not found {config_file}"
    ...
    def load_dataset(self):
        ...
        if "bbh" in self.data_path:
            ...
            with open(train_valid_file_path) as fin:
                ...
                dev_size = 0 # len(data) - train_size
            ...
            for split in ["train", "dev", "test"]:
                ...
                if self.n_test is not None and self.n_test < len(sentence_list) and split == "test":
                    indices = data_shuffling_rng.choice(
                        len(sentence_list), self.n_test, replace=False
                    )
                    for idx in indices:
            ...
            indices = np.arange(len(sentence_list))
            data_shuffling_rng.shuffle(indices)
            n_test = 1000
            n_dev = 0
    ...
    def get_hf_data(self):
        ...
        for split in hf_data:
            for sentence, label in zip(self.dataset[split]['sentence'], self.dataset[split]['label']):
                hf_data[split].append({
                    'sentence': sentence,
                    'label': int(label)
                })
        return hf_data
    
    def transform_label(self, mapping):
        """mapping: Dict[source label str: target label]"""
        for split in self.dataset:
            labels = [mapping[label] for label in self.dataset[split]['label']]
            self.dataset[split]['label'] = labels


def init_dataset(dataset_id, seed, data_dir, append_options=False, n_test=None):
    ...
    dataset = Dataset(dataset_location[dataset_id], dataset_id, seed, append_options=append_options, n_test=n_test)
